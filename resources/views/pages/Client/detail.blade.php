        ...
        <div class="card-body tab-content p-4">
            ...
            <div class="tab-pane fade {{ $activeTab == 'info' ? 'show active' : '' }}" id="info">
                <div class="row g-3">
                    ...
                    ['label'=>'Alamat','icon'=>'fa-location-dot','value'=> ($client->address ?? '-') . ', ' .
                    ($client->city ?? '') . ', ' . ($client->province ?? '') . ' ' . ($client->postcode ?? '')],
                    ['label'=>'Telepon','icon'=>'fa-phone','value'=>$client->phone ?? '-'],
                    ['label'=>'Email','icon'=>'fa-envelope','value'=>$client->email ?? '-'],
                    ['label'=>'Gender','icon'=>'fa-venus-mars','value'=> ucfirst($client->gender) ?? '-'],
                    ['label'=>'Pekerjaan','icon'=>'fa-briefcase','value'=>$client->job ?? '-'],
                    ['label'=>'Status','icon'=>'fa-user-check','value'=> ucfirst($client->marital_status) ?? '-'],
                    ['label'=>'Tipe','icon'=>'fa-building-user','value'=> $client->type == 'company' ? 'Perusahaan' :
                    'Pribadi'],
                    ];
                    @endphp
            ...
            <div class="tab-pane fade {{ $activeTab == 'dokumen' ? 'show active' : '' }}" id="dokumen">
                @include('pages.Client.partials.tab-dokumen', ['documents'=>$documents,
            ...
            <div class="tab-pane fade {{ $activeTab == 'tracking' ? 'show active' : '' }}" id="tracking">
                ...
                @include('pages.Client.partials.tab-tracking', ['picDocuments'=>$picDocuments])
                @else
                <div class="text-center text-muted py-5">
                    <i class="fas fa-map-marker-alt fa-2x mb-2"></i>
                    <p>Data tidak ditemukan untuk kode registrasi ini.</p>
                </div>
                @endif
                @else
                <div class="text-center text-muted py-5">
                    <i class="fas fa-map-marker-alt fa-2x mb-2"></i>
                    <p>Masukkan kode registrasi untuk melihat status</p>
                </div>
                @endif
            </div>
            ...
            <div class="tab-pane fade {{ $activeTab == 'pajak' ? 'show active' : '' }}" id="pajak">
                @include('pages.Client.partials.tab-pajak', ['notaryPayment'=>$notaryPayment,
